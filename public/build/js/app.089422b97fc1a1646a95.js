...
/***/ function(module, exports, __webpack_require__) {
	...
	function ThemeConfig($mdThemingProvider, mdcDatetimePickerDefaultLocaleProvider) {
	    ...
	    mdcDatetimePickerDefaultLocaleProvider.setDefaultLocale(window.NEXO.lang);
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	function MomentRun($rootScope, $translate, amMoment, $timeout) {
	    ...
	    $timeout(function () {
	    	...
	    	amMoment.changeTimezone("America/Bogota");
	    	//console.log(amMoment);
	    	//console.log(window.NEXO.lang);
	        //return amMoment.changeLocale($translate.use());
	        return amMoment.changeLocale(window.NEXO.lang);
	    });
...
/***/ function(module, exports) {
	...
	//module.exports = path;
	module.exports = window.NEXO.template_url+'admin.breadcrumb';
	    ...
	    $scope.items = new kendo.data.DataSource({
	        ...
	        transport: {
	            read: {
	                type: "get",
	                dataType: "json",
	                url: "http://nexo.nexologistica.com/api/customers?t=1468534255054&include=addresses&token=" + window.NEXO.token + "&team_id=zean8qpejzr5gxlepm39a4kbo7d6rq"
	            }
	        },
	        schema: {
	    ...
	    $scope.openResizeImageDialog = function () {
	        if ($scope.image) {
	            ...
	            $mdDialog.show({
	                ...
	                $scope.model.avatar = croppedDataUrl;
...
/***/ function(module, exports, __webpack_require__) {
	...
	function Controller($scope, uiGmapIsReady, uiGmapGoogleMapApi, $timeout) {
	    ...
	    var latitude = 4.7107325;
	    var longitude = -74.0669895;
	    ...
	    $scope.model.latitude = latitude;
	    $scope.model.longitude = longitude;
		    ...
		    uiGmapGoogleMapApi.then(function (maps) {
		        ...
		        $scope.marker.events = {
		                dragend: function dragend(marker, eventName, args) {
		                    ...
		                    console.log('place dragend lat '+lat+' lon '+lon);
				...
				// $scope.$watch('address.address', function(address) {
				// console.log('auto complete');;
				// if (address) {
				// 		var map = maps;
                //           var newMarkerLocation = _.get(
                //             address,
                //             'place.geometry.location'
				// 		  );
				// 		  console.log(newMarkerLocation.lat());
                //           if (newMarkerLocation) {
                //             center = new google.maps.LatLng(
                //               newMarkerLocation.lat(),
                //               newMarkerLocation.lng()
                //             );
                ...
                //             $scope.model.latitude = angular.copy(
                //               $scope.marker.coords.latitude
                //             );
                //             $scope.model.longitude = angular.copy(
                ...
                //               address.components.countryCode;
			 ...
			 // setTimeout(function () {
                ...
                .then(function(instances) {
					...
					// 	function(nv) {
					// 	_.set($scope.address, 'address', nv);
					// 	unwatch();
					// 	}
					// );

					// console.log('MODEL UNMATCH ', unwatch);
					// console.log('MODEL KEY ', 'model.' + $scope.options.key);
					});
                })
			    .catch(err => console.log('err', err));
						// },500);
							/*setTimeout(function () {
								angular.forEach(maps, function (v, key) {
										var map = v,
											center = new google.maps.LatLng($scope.model.latitude, $scope.model.longitude);

										//google.maps.event.trigger(map, 'resize');
										//$scope.map.id = $scope.model.id;
										if($scope.model.latitude){
											$scope.map.center = {
												latitude: $scope.model.latitude,
												longitude: $scope.model.longitude
    	...
    	/*$scope.$watch('address', function (address) {
    		...
    		if (address) {
    			...
    			if(newMarkerLocation == undefined){
    				...
    				$scope.model.latitude = angular.copy(lat);
    				$scope.model.longitude = angular.copy(lon);
    				$scope.model.is_autocompleted = 1;

    			}else if (newMarkerLocation.length > 0) {
    				//$scope.map.panTo(newMarkerLocation);
    				var lat = newMarkerLocation.lat();
    				var lon = newMarkerLocation.lng();
...
/***/ function(module, exports, __webpack_require__) {
	...
	DialogEditAddressController.$inject = ["$log", "$scope", "$mdDialog", "addressId","address","Store", "CustomerForm","$timeout"];
	Object.defineProperty(exports, "__esModule", {
	    value: true
	});
	...
	function Controller(Store, $scope, $mdDialog, $timeout) {
	    ...
	    $scope.editAddress = function (ev, index, address) {
	        $mdDialog.show({
	        	...
	        	console.log('I get called on escape and clickoutside editAddress');
	        	loadAddresses(address.customer_id);
	        });
	    };
	    ...
	    function loadAddresses(customerId) {
	        console.log('--- Loading addresses ==> load', customerId);

	        $scope.loading = true;
	        $scope.addresses = [];


	        Store.mapper('address').all(customerId).then(function (response) {
	        	$scope.$apply(function () {
	        		//$scope.$apply(function () {
	        		$scope.addresses = _lodash2.default.get(response, 'data', response);
	        		//console.log('2', $scope.addresses);
	        		$scope.loading = false;
	        		//});
	        		console.log('--- Loaded addresses ==> then ', $scope.addresses);
	    		});
	        }, function (errorResponse) {
	        	console.error('--- Loaded addresses ==> error ', errorResponse);
	        		$scope.loading = false;
	        });
	...
	function DialogEditAddressController($log, $scope, $mdDialog, addressId,address, Store, CustomerForm,$timeout) {
	    "ngInject";

	    var customerId = address.customer_id;
	    var address = {};


		//console.error('--- Loaded addresses edit ==> error ', address);
	    //var address = _store.store.get('address', addressId);
	    Store.mapper('address').search(customerId, addressId).then(function (response) {
	    	$scope.$apply(function () {
	    		address = response.data;
	    		//console.error('--- Loaded addresses edit ==> complete ', response);
	    	...
	    	console.error('--- Loaded addresses error  ==> mapper ', errorResponse);
	    });

	    //var address  = Store.get('address', addressId);
	    //console.error('--- Loaded addresses edit ==> complete ', address);
	    ...
	    $scope.save = function () {
	        ...
	        Store.onUpdate('address', address.id, $scope.model, {
	            params: {
		            ...
		            $scope.model = _lodash2.default.pick(response, ['place_id', 'address', 'latitude', 'longitude', 'street', 'street_number', 'city','map', 'district', 'state', 'country', 'country_code', 'observations', 'vicinity', 'is_autocompleted']);
...
/* 350 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.modals.address';
...
/* 351 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.modals.dialog';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var adapter = exports.adapter = new _jsDataHttp.HttpAdapter({
	    ...
	    beforeHTTP: function beforeHTTP(config) {
	        ...
	        //filter dates
	        if(data){
	        	if (_.has(data, 'recurrence_start')){
	        		if (!_.isUndefined(data.recurrence_start)){
	        			console.log('beforeHTTP');
	        			console.log(data.recurrence_start);
	        			data.recurrence_start =  (0, _moment2.default)(data.recurrence_start).format(format);
	        		}
	        	}
	        }
	...
	store.defineMapper('address', {
	    ...
	    search: function search(customerId, addressId) {
	        return store.getAdapter('http').POST('/api/customers/' + customerId + '/' + this.endpoint + '/' + addressId+ '/search');
	    },

	    all: function search(customerId) {
	        return store.getAdapter('http').GET('/api/customers/' + customerId + '/' + this.endpoint + '/');
	    }
	});
	...
	store.defineMapper('tool', {
	    endpoint: 'tools',
	    search: function search(data) {
	        return store.getAdapter('http').POST('/api/tools/search', data);
	    },
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _app5 = __webpack_require__(398);
	...
	function Config($translateProvider) {
	    ...
	    //$translateProvider.translations('en', _app6.default);
...
/* 402 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.menu';
...
/***/ function(module, exports, __webpack_require__) {
	...
	function Controller($rootScope, $translate, NEXO, Store, Toasts, $mdDialog, $window) {
	    ...
	    if (this.isTeam) {
	        ...
	        var tool = {
	                name: 'HERRAMIENTAS.HERRAMIENTAS',
	                url: 'herramientas'
	            };
	        ...
	        if(roles.indexOf('despachador') !== -1){
	        //if(roles.indexOf('customer') !== -1){
	        	...
	        	vm.menu.push(configuration);

	        }else if(roles.indexOf('rutero') !== -1){

	        }else{
	        		var modules = NEXO.team.modules;

	        		angular.forEach(modules, function (value, key) {
		        		if(value.id == 2 && value.active == true){
		        			configuration.children.push(tool);
		        		}
		        	});
		        	vm.menu.push(configuration);

	        		angular.forEach(modules, function (value, key) {
		        		if(value.id == 1 && value.active == true){
		        			vm.menu.push(statistics);
		        		}
		        	});
	    ...
	    vm.currentLang = window.NEXO.lang;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var headerComponent = {
	    restrict: 'E',
	    templateUrl: _headerComponent2.default,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var nxTableComponent = {
	    ...
	    templateUrl: _nxTableComponent2.default,
...
/* 420 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.table';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var component = {
	    restrict: 'E',
	    templateUrl: _assignmentDetailComponent2.default,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Controller = function Controller(Store,$http,$scope) {
	    "ngInject";
	    ...
	    if (_.isUndefined(vm.item.id)){
		    if (!_.isUndefined(vm.item.duration)){
		    	duration = vm.item.duration;
		    }
		    if (vm.item.start_at){
		    	vm.item.start_at = (0, _moment2.default)(vm.item.start_at).toDate();
		    }
		    if (vm.item.end_at){
		    	if(duration){
		    		vm.item.end_at = (0, _moment2.default)(vm.item.start_at).add(duration, 'minutes').toDate();
		    	}else{
		    		vm.item.end_at = (0, _moment2.default)(vm.item.end_at).toDate();
		    	}
		    }
		    if (vm.item.recurrence_start) {
		    	vm.item.recurrence_start = (0, _moment2.default)(vm.item.recurrence_start).toDate();
		    }
		    if (vm.item.recurrence_end){
		    	vm.item.recurrence_end = (0, _moment2.default)(vm.item.recurrence_end).toDate();
		    }
		}
	    ...
	    Store.mapper('tool').search(vm.item).then(function (response) {
	    	if(response.status == 200){
	    		$scope.$apply(function () {
	    			vm.item.tools = response.data;
	    		});
	    	}
	    }, function (errorResponse) {
	            vm.item.fixing = false;
	            vm.item.errorFixing = true;
	     });

	    console.log('Detail assing');
	    console.log(vm.item);

	};
	Controller.$inject = ["Store","$http","$scope","$timeout", "NEXO"];
...
/***/ function(module, exports) {
	...
	module.exports = "<ui-gmap-google-map center=\"map.center\" zoom=\"map.zoom\" draggable=\"true\" options=\"map.options\">\n<ui-gmap-markers models=\"vm.dashboardMarkers\" coords=\"'self'\" icon=\"'icon'\" doCluster=\"true\" options=\"'options'\" click=\"onClickUser\"></ui-gmap-markers>\n</ui-gmap-google-map>";
...
/***/ function(module, exports, __webpack_require__) {
	...
	var DashboardMapController = function () {
	    DashboardMapController.$inject = ["$scope", "uiGmapIsReady", "uiGmapGoogleMapApi", "$timeout", "Store","$mdDialog"];
	    function DashboardMapController($scope, uiGmapIsReady, uiGmapGoogleMapApi, $timeout, Store,$mdDialog) {
	        "ngInject";
		    ...
		    // Cuando damos click desde el mapa sobre un rutero
		    //
		    $scope.onClickUser = function (ev, evName, item) {
		        $scope.openUserDialog(ev, item.user);
		    };
		    ...
		    $scope.openUserDialog = function (ev, user) {
		    	...
		    	//enviar modelo con la asignacion 364
		    ...
		    console.log(vm.dashboardMarkers.length);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _index23 = __webpack_require__(730);

	var _index24 = _interopRequireDefault(_index23);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	exports.default = _angular2.default.module('app.pages', [_index2.default.name, _index4.default.name, _index6.default.name, _index8.default.name, _index10.default.name, _index12.default.name, _index14.default.name, _index16.default.name, _index18.default.name, _index20.default.name, _index22.default.name, _index24.default.name]);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var dashboardPage = {
	    ...
	    templateUrl: _dashboardPage2.default,
...
/***/ function(module, exports) {
	...
	module.exports = window.NEXO.template_url+'admin.dashboard';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var component = {
	    restrict: 'E',
	    templateUrl: _formComponent2.default,
...
/* 485 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.assignments.step';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Controller = function () {
	    Controller.$inject = ["NEXO", "Store", "Dialogs", "AssignForm", "WizardHandler", "$timeout", "$log", "$scope"];
	    function Controller(NEXO, Store, Dialogs, AssignForm, WizardHandler, $timeout, $log,$scope) {
	        "ngInject";
	        ...
	        if (!_.isEmpty(vm.model)) {
	            ...
	            //vm.model.start_at = (0, _moment2.default)(vm.model.start_at).toDate();
	            //vm.model.end_at = (0, _moment2.default)(vm.model.end_at).toDate();
	    ...
	    (0, _createClass3.default)(Controller, [{
	        ...
	        value: function onSubmit() {
	            ...
	            if (model.start_at){
	                model.start_at = (0, _moment2.default)(model.start_at).format(format);
	            }
	            if (model.end_at){
	                model.end_at = (0, _moment2.default)(model.end_at).format(format);
	            }

	            if (model.recurrence_start) {
	                model.recurrence_start = (0, _moment2.default)(model.recurrence_start).format(format);
	            }
	            if (model.recurrence_end){
	                model.recurrence_end = (0, _moment2.default)(model.recurrence_end).format(format);
	            }

	            console.log('new / update assing ', model);
...
/* 520 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.clients.view';
	...
	var ProductosController = function ProductosController() {
	    ...
	    vm.options = {
	        ...
	        actions: {
	            ...
	            categories: 'productos.categorias.crear',
	            header: [{
...
/***/ function(module, exports, __webpack_require__) {
	...
	var crearPage = {
	    restrict: 'E',
	    bindings: {},
	    templateUrl: _crearPage2.default,
	    controller: _crearPage4.default,
	    controllerAs: 'vm'
	};
...
/* 542 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.products.categories.edit';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var EditarPage = {
	    restrict: 'E',
	    bindings: {},
	    templateUrl: _editarPage2.default,
	    controller: _editarPage4.default,
	    controllerAs: 'vm'
	};
...
/* 557 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.products.edit';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var importarPage = {
	    restrict: 'E',
	    bindings: {},
	    templateUrl: _importarPage2.default,
	    controller: _importarPage4.default,
	    controllerAs: 'vm'
	};
...
/* 567 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.products.import';
...
/* 572 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.products.view';
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.default = _angular2.default.module('estadisticas.encuestas.page', [_angularUiRouter2.default]).config(["$stateProvider", function ($stateProvider) {
	    "ngInject";
...
/***/ function(module, exports) {
	...
	module.exports = "<md-toolbar class=\"md-toolbar-grey\">\n    <div class=\"md-toolbar-tools\">\n        <h3>\n            <span>{{ \"PAGES.ESTADISTICAS.ENCUESTAS.TITLE\" | translate }}</span>\n        </h3>\n    </div>\n</md-toolbar>\n\n\n<md-content layout-padding layout=\"column\">\n\n    <div flex layout=\"row\" layout-wrap layout-align=\"space-between center\">\n        <md-menu>\n            <md-button aria-label=\"Menú para exportar\" class=\"md-raised\" ng-click=\"$mdOpenMenu($event)\">\n                <span md-menu-origin>{{ \"PAGES.ESTADISTICAS.RUTEROS.EXPORTAR\" | translate }}</span>\n            </md-button>\n            <md-menu-content width=\"4\">\n                <md-menu-item>\n                    <md-button ng-click=\"vm.exportChart('image')\">\n                        <md-icon md-svg-icon=\"file-image\" md-menu-align-target></md-icon>\n                        {{ \"PAGES.ESTADISTICAS.RUTEROS.GRAFICO_A_IMAGEN\" | translate }}\n                    </md-button>\n                </md-menu-item>\n                <md-menu-item>\n                    <md-button ng-click=\"vm.exportChart('pdf')\">\n                        <md-icon md-svg-icon=\"file-pdf\" md-menu-align-target></md-icon>\n                        {{ \"PAGES.ESTADISTICAS.RUTEROS.GRAFICO_A_PDF\" | translate }}\n                    </md-button>\n                </md-menu-item>\n                <md-menu-item>\n                    <md-button ng-click=\"vm.exportTable('excel')\">\n                        <md-icon md-svg-icon=\"file-excel\" md-menu-align-target></md-icon>\n                        {{ \"PAGES.ESTADISTICAS.RUTEROS.TABLA_A_EXCEL\" | translate }}\n                    </md-button>\n                </md-menu-item>\n                <md-menu-item>\n                    <md-button ng-click=\"vm.exportTable('pdf')\">\n                        <md-icon md-svg-icon=\"file-pdf\" md-menu-align-target></md-icon>\n                        {{ \"PAGES.ESTADISTICAS.RUTEROS.TABLA_A_PDF\" | translate }}\n                    </md-button>\n                </md-menu-item>\n\n            </md-menu-content>\n        </md-menu>\n\n        <form  name=\"vm.form\" novalidate flex  ng-submit=\"vm.submit()\" layout=\"row\" layout-align=\"end center\">\n            <div>\n                <formly-form model=\"vm.model\" fields=\"vm.fields\" form=\"vm.form\"></formly-form>\n            </div>\n            <div>\n                <md-button type=\"submit\" class=\"md-raised nd-default\" ng-disabled=\"vm.loading\">{{ \"PAGES.ESTADISTICAS.RUTEROS.FILTRAR\" | translate }}</md-button>\n            </div>\n        </form>\n    </div>\n\n\n\n    <div class=\"mt\">\n       <div class=\"mt-l\" id=\"grid\"></div>\n    </div>\n</md-content>\n";
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Controller = function () {
	    ...
	    function Controller(StatsForm, $http, $timeout, NEXO) {
	        ...
	        var statForm = StatsForm;

	        statForm[0].fieldGroup[2].className = 'flex hide';
	            ...
	            $http({
	                ...
	                if (!_.isEmpty(series.data)) {
	                    ...
	                    $grid.kendoGrid({
	                        ...
	                        excel: {
	                            author: "Nexo Logística",
	                            fileName: "tabla-encuestas.xlsx"
	                        },
	                        pdf: {
	                            author: "Nexo Logística",
	                            creator: "Nexo Logística",
	                            fileName: "tabla-encuestas.pdf"
	                        },
	                        groupable: false,
	                        sortable: true,
	                        resizable: true,
	    ...
	    (0, _createClass3.default)(Controller, [{
	        ...
	        value: function exportChart() {
	            ...
	            chart.options.title.text = 'Resultados de encuestas';
	            ...
	            chartExport.done(function (data) {
	                kendo.saveAs({
	                    dataURI: data,
	                    fileName: "resultados-encuestas." + (type == 'image' ? 'png' : 'pdf')
	                });
...
/***/ function(module, exports) {
	...
	module.exports = window.NEXO.template_url+'admin.polls.view';
	...
	var CrearController = function () {
	    ...
	    (0, _createClass3.default)(CrearController, [{
	        ...
	        value: function onSubmit() {
	            ...
	            var model = _lodash2.default.omit(vm.model, []);
...
/* 627 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.services.edit';
	/*module.exports = "<form name=\"vm.form\" autocomplete=\"off\" ng-submit=\"vm.onSubmit()\" novalidate autocomplete=\"off\">\n\n    <md-toolbar class=\"md-toolbar-grey\">\n        <div class=\"md-toolbar-tools\">\n            <span flex></span>\n\n            <md-button md-theme=\"nexo\" type=\"submit\" class=\"md-raised md-primary\">Guardar</md-button>\n            <md-button ui-sref=\"productos.categorias\" class=\"md-raised nd-default\">Cancelar</md-button>\n        </div>\n    </md-toolbar>\n\n\n    <md-content layout=\"row\" layout-padding>\n        <div flex=\"50\">\n            <h3 class=\"panel-title\">Información básica</h3>\n            <formly-form model=\"vm.model\" fields=\"vm.fields.basic\" form=\"vm.form\"></formly-form>\n        </div>\n    </md-content>\n</form>";*/
...
/* 649 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.polls.edit';
	//module.exports = "<form name=\"vm.form\" autocomplete=\"off\" ng-submit=\"vm.onSubmit()\" novalidate autocomplete=\"off\">\n\n    <md-toolbar class=\"md-toolbar-grey\">\n        <div class=\"md-toolbar-tools\">\n            <span flex></span>\n\n            <md-button md-theme=\"nexo\" type=\"submit\" class=\"md-raised md-primary\">Guardar</md-button>\n            <md-button ui-sref=\"encuestas\" class=\"md-raised nd-default\">Cancelar</md-button>\n        </div>\n    </md-toolbar>\n\n\n    <md-content layout=\"row\" layout-padding>\n        <div flex=\"50\">\n            <h3 class=\"panel-title\">Información básica</h3>\n            <formly-form model=\"vm.model\" fields=\"vm.fields.basic\" form=\"vm.form\"></formly-form>\n        </div>\n    </md-content>\n</form>";
	    ...
	    $stateProvider.state({
	        ...
	        resolve: {
	            item: /*@ngInject*/["Store", "$stateParams", function item(Store, $stateParams) {
	                ...
	                return Store.find('poll', $stateParams.id).then(function (response) {
	                    return response;
	                });
	            }]
	        }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _tool = __webpack_require__(682);
...
/***/ function(module, exports, __webpack_require__) {
	...
	function Form(NEXO, $translate) {
	    ...
	    return {
	        step1: [
	        {
	        	hideExpression: '!model.service_status_id',
	            key: 'service_status_id',
	            type: 'select',
	            templateOptions: {
	            	label: 'FORMS.TEAM.ESTADO',
	            	required: true,
	            	valueProp: 'value',
	            	labelProp: 'name',
	            	options: NEXO.statuses_options
	            },
	            defaultValue: '1',
	        ...
	        step2: [{
	            ...
	            controller: /*@ngInject*/["Store", "$scope", "$filter", function controller(Store, $scope, $filter) {
	                    ...
	                    //
	                    $scope.$watch('model.with', function (value) {

	                    	if(value != undefined){
	                    		if(value == 'services'){
	                    			if($scope.model.id == undefined){
	                    				$scope.model.services = [];
	                    				$scope.model.duration_service = 0;
	                    			}

	                    			$scope.model.duration = $scope.model.duration_service;
	                    		}else if(value == 'products'){
	                    			if($scope.model.id == undefined){
	                    				$scope.model.duration_product = 15;
	                    			}
	                    			$scope.model.duration = $scope.model.duration_product;
	                    		}else if(value == 'both'){
	                    			if($scope.model.id == undefined){
	                    				$scope.model.services = [];
	                    				$scope.model.duration_service = 0;
	                    			...
	                    			$scope.model.duration = $scope.model.duration_product+$scope.model.duration_service;
	                    		}
	                    	}
	                    });
	            }],
			...
			},
			{
	            ...
	            type: 'textarea',
	            templateOptions: {
	            	label: 'PAGES.ASIGNACIONES.OBSERVACIONES',
	            	required: false
	            },
	            defaultValue: '',
	            controller: /*@ngInject*/["$scope", function controller($scope) {
	                    ...
	                    $scope.$watch('model.services', function (servicesIds) {
	                        ...
	                        if (servicesIds.length <= 1) {
	                            if (!_.isEmpty(services)) {
	                                _.set($scope, 'model.duration_service', _.first(services).duration.minutes);
	                                //_.set($scope, 'model.duration', _.first(services).duration.minutes);

	                                var value = $scope.model.with;

	                                if(value != undefined){
	                                	if(value == 'services'){
	                                		$scope.model.duration = $scope.model.duration_service;
	                                	}else if(value == 'products'){
	                                		$scope.model.duration_product = 0;
	                                		$scope.model.duration = $scope.model.duration_product;
	                                	}else if(value == 'both'){
	                                		$scope.model.duration = $scope.model.duration_product+$scope.model.duration_service;
	                                	}
	                    ...
	                    $scope.$watch('model.services', function (servicesIds) {
	                        if (servicesIds) {
	                            ...
	                            if (!_.isEmpty(services)) {
	                                ...
	                                if(value != undefined){
	                                	if(value == 'services'){
	                                		$scope.model.duration_service = sumDuration;
	                                		$scope.model.duration = $scope.model.duration_service;
	                                	}else if(value == 'products'){
	                                		$scope.model.duration_product = 0;
	                                		$scope.model.duration = $scope.model.duration_product;
	        ...
	        // Cuando son productos
	        {
	            ...
	            fieldGroup: [{
	                ...
	                controller: /*@ngInject*/["Store", "$scope", "$filter", function controller(Store, $scope, $filter) {
	                    ...
	                    // Opciones para elegir si la duración es con el total o el máximo
	                    //
	                    $scope.$watch('model.duration_product', function (duration) {
	                    	...
	                    	if(value != undefined){
	                    		if(value == 'services'){
	                    			...
	                    			$scope.model.duration_product = duration;
	                    			$scope.model.duration = $scope.model.duration_product;
	                    		}else if(value == 'both'){
	                    			$scope.model.duration_product = duration;
	                    			$scope.model.duration = $scope.model.duration_product+$scope.model.duration_service;
	                        ...
	                        $scope.model[$scope.options.key] = duration;
	                    });
	                }]
	            }, {
	                key: 'products',
	            ...
	            fieldGroup: [{
	                ...
	                controller: /*@ngInject*/["Store", "$scope", function controller(Store, $scope) {
	                    $scope.$watchGroup(['model.date_type', 'model.duration'], function (nv) {
	                        ...
	                        if (dateType == 'recurrent') {
	                            ...
	                            _.set($scope, 'model.assignment_type', 'mandatory');
	                            ...
	                            var _unwatchRecurrence = $scope.$watchGroup(recurrenceModelForWatch, function (nv) {
	                                ...
	                                console.log('set recurrent');
	                                _.set($scope, 'model.start_at', (0, _moment2.default)($scope.model.recurrence_start).toDate());
	                            ...
	                            if (dateType == 'schedule') {
	                            	_.set($scope, 'model.assignment_type', 'demand');
	                            }

	                            if (dateType == 'inmediate') {
	                            	_.set($scope, 'model.assignment_type', 'demand');
	                            }
	                ...
	                expressionProperties: {
	                	'templateOptions.options': function($viewValue, $modelValue, $scope) {
				           var options = NEXO.assignments_options.types;
				           console.log(options)
				           //$scope.to.options=options;
				           if($scope.model.date_type == 'recurrent'){
				           	options = [{"name":"Obligatorio","value":"mandatory"}];
				           	return options;
				           }else{
				           	return options;
				           }
	...
	function Form(IS_TEAM, $translate) {
	    ...
	    function RoleFieldController($scope, Store) {
	        ...
	        $scope.to.options = Store.filter('role', {
	            where: { for_team: IS_TEAM, active: 1 }
	        });
	    	...
	    	basic: [
			    	{
			    		...
			    		controller: /*@ngInject*/["$scope", "Store", "$timeout", function controller($scope, Store, $timeout) {
			    			$scope.$watch('model.services', function (rservicesIds) {
			    				var data = rservicesIds.data;
			    				var servicesIds = [];
			    				...
			    				if(data){
			    					angular.forEach(data, function (v, k) {
			    						//console.log(v);
			    						console.log(v);
			    						servicesIds.push(v.id);
			    					});
			    				}else{
			    					servicesIds = rservicesIds
			    				}


								/*var services = {};

			    				services.data = _store.store.filter('serviceType', {
			    					where: {
			    						id: {
			    							'in': servicesIds
			    						}
			    				...
			    				// Settear services_data para resumen
			    				_.set($scope, 'model.services', servicesIds);
			    		...
			    		type: 'multiselect',
			    		templateOptions: {
			    			label: 'FORMS.ASSIGN.PRODUCTOS',
			    			valueProp: 'product_id',
			    			labelProp: 'name',
			    			options: _store.store.filter('product', {}),
			    			required: false
			    		},
			    		controller: /*@ngInject*/["$scope", "Store", "$timeout", function controller($scope, Store, $timeout) {
			    			$scope.$watch('model.products', function (rproductsIds) {
			    				var data = rproductsIds.data;
			    				var productsIds = [];
			    				...
			    				// Settear services_data para resumen
			    				_.set($scope, 'model.products', productsIds);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _herramientas = __webpack_require__(731);

	var _herramientas2 = _interopRequireDefault(_herramientas);

	var _index = __webpack_require__(735);

	var _index2 = _interopRequireDefault(_index);

	var _index3 = __webpack_require__(745);

	var _index4 = _interopRequireDefault(_index3);

	var _ver = __webpack_require__(760);
	...
	var herramientasPageModule = _angular2.default.module('herramientas.page', [_angularUiRouter2.default, _index2.default.name, _index4.default.name, _ver2.default.name]).config(["$stateProvider", function ($stateProvider) {
	    "ngInject";
	    ...
	    $stateProvider.state({
	        ...
	        ncyBreadcrumb: {
	            parent: 'dashboard',
	            label: 'PAGES.HERRAMIENTAS.TITLE'
	        }
	    });
	}]).component('herramientasPage', _herramientas2.default);

	exports.default = herramientasPageModule;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _herramientasPage = __webpack_require__(732);

	var _herramientasPage2 = _interopRequireDefault(_herramientasPage);

	var _herramientasPage3 = __webpack_require__(733);

	var _herramientasPage4 = _interopRequireDefault(_herramientasPage3);

	__webpack_require__(734);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	var herramientasPage = {
	    restrict: 'E',
	    bindings: {},
	    templateUrl: _herramientasPage2.default,
	    controller: _herramientasPage4.default,
	    controllerAs: 'vm'
	};

	exports.default = herramientasPage;

/***/ },
/* 732 */
/***/ function(module, exports) {
	module.exports = window.NEXO.template_url+'admin.tools.table';
	//module.exports = "<nx-table options=\"vm.options\"></nx-table>";
...
/***/ function(module, exports, __webpack_require__) {
	...
	var HerramientasController = function HerramientasController() {
	    "ngInject";

	    (0, _classCallCheck3.default)(this, HerramientasController);
	    var vm = this;
	    ...
	    vm.options = {
	        ...
	        actions: {
	            create: 'herramientas.crear',
	            edit: 'herramientas.editar',
	            show: 'herramientas.ver',
	            header: []
	        }
	    };
	};

	exports.default = HerramientasController;
...
/***/ function(module, exports,__webpack_require__) {
	...
	var _crear = __webpack_require__(736);

	var _crear2 = _interopRequireDefault(_crear);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	exports.default = _angular2.default.module('herramientas.crear.page', [_angularUiRouter2.default]).config(["$stateProvider", function ($stateProvider) {
	    "ngInject";

	    $stateProvider.state({
	        name: 'herramientas.crear',
	        url: '/crear',
	        views: {
	            "@": {
	                component: 'herramientasCrearPage'
	            }
	        },
	        resolve: {
	            serviceType: /*@ngInject*/["Store", function serviceType(Store) {
	            ...
	            products: /*@ngInject*/["Store", function products(Store) {
	                return Store.findAll('product', {
	                    t: _.now()
	                });
	            }]
	        },
	...
	}]).component('herramientasCrearPage', _crear2.default);
...
/***/ function(module, exports, __webpack_require__) {
		...
		var _crearPage = __webpack_require__(737);

		var _crearPage2 = _interopRequireDefault(_crearPage);

		var _crearPage3 = __webpack_require__(738);
...
/* 737 */
/***/ function(module, exports) {
		module.exports = window.NEXO.template_url+'admin.tools.edit';
/***/ },
/* 738 */
/***/ function(module, exports, __webpack_require__) {
	...
	var CrearController = function () {
	    CrearController.$inject = ["$translate", "Store", "ToolForm"];
	    function CrearController($translate, Store, ToolForm) {
	        "ngInject";
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _editar = __webpack_require__(746);
	...
	exports.default = _angular2.default.module('herramientas.editar.page', [_angularUiRouter2.default]).config(["$stateProvider", function ($stateProvider) {
	    "ngInject";

	    $stateProvider.state({
	        name: 'herramientas.editar',
	        url: '/:id/editar',
	        views: {
	            "@": {
	                component: 'herramientasEditarPage'
	            }
	        },
	        ncyBreadcrumb: {
	            label: 'Editar {{$resolve.item.name}}'
	        ...
	        resolve: {
	        	item: /*@ngInject*/["$stateParams", function item($stateParams) {
	                return _store.store.find('tool', $stateParams.id, {
	                    params: {
	...
	}]).component('herramientasEditarPage', _editar2.default);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _editarPage = __webpack_require__(747);

	var _editarPage2 = _interopRequireDefault(_editarPage);

	var _editarPage3 = __webpack_require__(748);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var CrearController = function () {
	    CrearController.$inject = ["Store", "ToolForm"];
	    function CrearController(Store, ToolForm) {
	        "ngInject";
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ver = __webpack_require__(761);

	var _ver2 = _interopRequireDefault(_ver);

	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

	exports.default = _angular2.default.module('herramientas.ver.page', [_angularUiRouter2.default]).config(["$stateProvider", function ($stateProvider) {
	    "ngInject";

	    $stateProvider.state({
	        name: 'herramientas.ver',
	        url: '/:id',
	        views: {
	            "@": {
	                component: 'herramientasVerPage'
	            }
	        },
	        resolve: {
	            item: /*@ngInject*/["Store", "$stateParams", function item(Store, $stateParams) {
	                return Store.find('tool', $stateParams.id).then(function (response) {
	                    return response;
	                });
	            }]
	        },
	...
	}]).component('herramientasVerPage', _ver2.default);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _verPage = __webpack_require__(762);

	var _verPage2 = _interopRequireDefault(_verPage);

	var _verPage3 = __webpack_require__(763);
...
/* 762 */
/***/ function(module, exports) {
			module.exports = window.NEXO.template_url+'admin.tools.view';
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Controller = function Controller(Store, $stateParams) {
	    ...
	    vm.item = Store.get('tool', $stateParams.id);
	};
	Controller.$inject = ["Store", "$stateParams"];
